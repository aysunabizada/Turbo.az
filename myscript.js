...
//daha cox filterleri gosterir
let flag = false;
function filterCoxalt() {
    flag = !flag;
    hiddeninp.style.display = flag ? "flex" : "none";
    textA.innerHTML = flag ? `Gizlət <i class="fa-solid fa-chevron-up"></i>` : `Daha çox filtr <i class="fa-solid fa-chevron-down"></i>`;
}
...
function yeniElan() {
    flag = !flag;
    yenielan.style.display = flag ? "block" : "none";
    main.style.display = flag ? "none" : "block";
    mainsearch.style.display = flag ? "none" : "block";
}
...
//footerdeki listleri cixarir
let kod = '';
for (let i = 0; i < data.cars.length; i++) {
    kod += `<a href="">${data.cars[i].model} (${data.cars[i].model.length})</a>`;
}
footerlist.innerHTML = kod;

//selectlere yigan andir
...
getOptions([...new Set(data.cars.map(item => item.city))], seher);
getOptions([...new Set(data.cars.map(item => item.currency))], valyuta);
getOptions([...new Set(data.cars.map(item => item.banType))], ban);
...
//barter ve kredit seyi
function kreditBarter(arg) {
    cards.innerHTML = '';
...
let f = true;
function like(i, id) {
    let likedCar =  data.cars.filter(item => id == item.id);
...
//desktop secilmislere geden
function favorite() {
    heart.style.color = '#CA1016';
    secilmisler.style.display = "flex";
    main.innerHTML = '';
    mainsearch.innerHTML = '';
    if (likedCars.length === 0) {
        secilmismain.style.display = 'flex';
    } else {
        secilmismain.style.display = 'none';
        for (let i = 0; i < likedCars.length; i++) {
            secilmisler.innerHTML += `
                <div class="card">
                    <div class="card-img">
                        <img src="${likedCars[i][0].images}" alt="">
                        <i class="fa-regular fa-heart" style="font-weight:bold; color: red;" onclick="like(this, ${likedCars[i][0].id}), sil(this, ${likedCars[i][0].id})"></i>
                        <span class="df">
                            <i class="fa-solid fa-crown"></i>
                            <i class="fa-solid fa-gem"></i>
                        </span>
                    ...
                    <div class="card-sum">
                        <h3>${likedCars[i][0].price} ${likedCars[i][0].currency}</h3>
                        <p>${likedCars[i][0].brand} ${likedCars[i][0].model}</p>
                        <p>${likedCars[i][0].year}, ${likedCars[i][0].engine}L, ${likedCars[i][0].odometer} ${likedCars[i][0].odometerUnit}</p>
                        <span>${likedCars[i][0].city}, bugun 16:23</span>
                    </div>
                </div>`
        }
    }
...
function sil(id) {
    console.log("salam");
    let index =  data.cars.findIndex(item => id == item.id);
...
function changePage(x){
    ...
    footer.style.display = 'block'
    if (x == 'like') {
        liked.style.color = '#ca1016'
        faheart.style.color = '#ca1016'
        home.style.color = 'initial'
        fahouse.style.color = '#8d94ad'
        h4.innerHTML = 'Seçilmişlər'
        cards.innerHTML = ''
        header.style.display = 'none'
        if (likedCars.length == 0) {
            cards.innerHTML += `
            <img src="img/fav.svg" alt="" />
            <p style="text-align: center;"> Bəyəndiyiniz elanları ürək işarəsinə klik edərək seçilmişlərə əlavə edin.</p>`
        }else{ 
            for (let i = 0; i < likedCars.length; i++) {
                cards.innerHTML += `
                    <div class="card">
...
//buttonlari qizardir
let flagg = true;
function bgRed(button) {
    if (flagg == true) {
        button.style.backgroundColor = "#FEE5E4";
        button.style.border = "1px solid #CA1016";
        button.style.color = "#CA1016";
        flagg = !flagg; 
    } else {
        button.style.backgroundColor = "#fff";
        button.style.border = "1px solid #dfe3e9";
